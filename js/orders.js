...
async function loadCustomerOrders() {
  ...
  try {
    ...
    orders.sort((a, b) => {
      // Handle Timestamp objects or simpler dates
    ...
    container.innerHTML = `
      <div class="empty-state">
        ...
        <p class="empty-state-text">เกิดข้อผิดพลาด: ${error.message}</p>
        <p style="font-size: 0.8em; color: gray; margin-top:5px;">Code: ${error.code || 'unknown'}</p>
        ${error.message.includes('index') ? '<p style="font-size: 0.8em; color: #e74c3c;">ระบบต้องการ Index (ดู Link ใน Console)</p>' : ''}
      </div>`;
  }
}
